...
def test_get_deps_builds_lazily_then_reuses_cache(tmp_path: Path) -> None:
    ...
    again = get_deps(_make_request(app))
    assert again is built


def test_get_deps_raises_500_when_config_path_is_none() -> None:
    app = create_app(
        session_token="T",
        csrf_secret="S",
        config_path=None,
    ...
    with pytest.raises(HTTPException) as exc_info:
        get_deps(_make_request(app))

    assert exc_info.value.status_code == 500


def test_get_deps_raises_500_when_config_path_does_not_exist(tmp_path: Path) -> None:
...
def test_get_deps_raises_500_when_repo_factory_is_none(tmp_path: Path) -> None:
    archive_root = tmp_path / "archive"
    archive_root.mkdir()
    config_path = tmp_path / "config.toml"
    _write_config(config_path, archive_root)
